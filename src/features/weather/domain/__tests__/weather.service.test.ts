...
const aRawForecast = (overrides?: RawHourly) => ({
  ...
  hourly: {
    time: overrides?.time ?? ["2026-06-25T18:00", "2026-06-25T19:00", "2026-06-26T09:00"],
    temperature_2m: overrides?.temperature_2m ?? [21.4, 20.1, 14.8],
...
describe("isOpenMeteoForecast", () => {
  ...
  it.each([
    ...
    ["tableaux vides", { hourly: { time: [], temperature_2m: [], precipitation_probability: [] } }],
    [
      "time non tableau",
      { hourly: { time: "x", temperature_2m: [1], precipitation_probability: [1] } },
    ],
    [
      "température non numérique",
      { hourly: { time: ["t"], temperature_2m: ["x"], precipitation_probability: [1] } },
    ],
    [
      "longueurs incohérentes",
      { hourly: { time: ["a", "b"], temperature_2m: [1], precipitation_probability: [1, 2] } },
    ],
...
describe("createWeatherForecast", () => {
  ...
  it("renvoie null pour une réponse invalide ou vide", () => {
    ...
    expect(
      createWeatherForecast(
        aRawForecast({ time: [], temperature_2m: [], precipitation_probability: [] }),
      ),
    ).toBeNull();
  });
});
...
describe("createSlotWeatherViewModel", () => {
  ...
  it("formate la météo de l'heure de début (aujourd'hui)", () => {
    ...
    expect(vm).toEqual({
      ...
      a11yLabel: "Météo : 21°, 30 % de risque de pluie",
    });
  });

  it("résout le bon jour pour « demain »", () => {
    const vm = createSlotWeatherViewModel(forecast, "tomorrow", "09:00", NOW);
    expect(vm?.temperatureLabel).toBe("15°");
    expect(vm?.precipitationLevel).toBe("low");
  });
  ...
  it("sélectionne par date réelle : une prévision périmée (fetch la veille) n'affiche pas la météo d'hier", () => {
    ...
    expect(today?.temperatureLabel).toBe("22°");
  ...
  it("arrondit les températures négatives", () => {
    ...
    expect(createSlotWeatherViewModel(cold, "today", "18:00", NOW)?.temperatureLabel).toBe("-4°");
  });
  ...
  it.each([
    ...
    expect(createSlotWeatherViewModel(f, "today", "18:00", NOW)?.precipitationLevel).toBe(expected);
  });
});
